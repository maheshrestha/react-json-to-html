...
function generateImportDeclaration(t, body, importedIdentifierName, localIdentifierName, sourceValue){
  ...
  if(!!importSourceNode.specifiers)
	importSourceNode.specifiers.push(t.ImportSpecifier(t.Identifier(importedIdentifierName), t.Identifier(localIdentifierName)))
  else {
  ...
  return {
    ...
    visitor: {
      ...
      Program(path, state) {
        ...
        generateImportDeclaration(t, b, setFieldNameToImport, setFieldNameToImport, '../ducks/filters');
      },
      FunctionDeclaration(path, state) {
        ...
        if(path.node.id.name === 'setFiltersFromUrlSaga') {
          ...
          const fieldDataType = state.opts.fieldDataType;
          path.node.body.body
          	.find(p => !!p.declarations && p.declarations.find(s => s.init.name === 'matches'))
          	.declarations.find(a => a.init.name === 'matches')
          	.id.elements.push(t.Identifier(camelCaseFieldName));
          if(fieldDataType === 'number') {
            path.node.body.body.push(t.Identifier(`yield put(${setFieldNameToImportMethod}(getStateReadyNumber(${camelCaseFieldName})));`));
          }
          else if(fieldDataType === 'string') {
            path.node.body.body.push(t.Identifier(`yield put(${setFieldNameToImportMethod}(getStateReadyString(${camelCaseFieldName})));`));
          }
          else if(fieldDataType === 'date') {
            path.node.body.body.push(t.Identifier(`yield put(${setFieldNameToImportMethod}(getStateReadyDate(${camelCaseFieldName})));`));
          }
