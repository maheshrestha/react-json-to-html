...
const getParameterForRecordSchema = (schema, property) => {
  ...
  for (const property in schema) {
    ...
    if (Array.isArray(schema[property])) {
      ...
      processStrategyEntity.push(`${propertyToCamelCaseString}Ids: [${capitalize(propertyToCamelCaseString)}Schema]`);
      ...
      processStrategyEntity.push(`${propertyToCamelCaseString}Id: ${capitalize(propertyToCamelCaseString)}Schema`);
  ...
  for (const property in schema) {
    ...
    if (!!schema[property] && typeof schema[property] === 'object') {
      ...
      return_params.push({
        ...
        destination: `../react-app/src/${toCamelCaseString(arguments.componentName)}/schemas/${capitalize(propertyToCamelCaseString)}Schema.js`,
        parameters: { 
