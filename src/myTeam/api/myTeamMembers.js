...
import MyTeamMembersApiSchema from '../schemas/MyTeamMembersApiSchema';

export async function fetchMyTeamMembers(queryParams: String = ''): Promise<*> {
  const url = `${DATA_READ_ENDPOINT_GET}?${queryParams}`;
  const response = await getJSON(url);
  if (response.status < 500 && typeof response.data === 'object') {
    return normalize(response.data, MyTeamMembersApiSchema);
  }
  return false;
}
...
export async function sendAddMyTeamMembers(
  props: Object
): Promise<*> {
  const data = {
    form: {
...
export async function sendRemoveMyTeamMembers(id: string): Promise<*> {
